...
$highestAccuracy = $averageAccuracy = 0;
$scores = [];

if (!empty($selectedTest)) {
    $stmt = $conn->prepare("
        SELECT rollno, batch, testname, right_question, wrong_question, not_attempted, max_marks, marks_obtained, percentage, award_for_wrong, award_for_right, accuracy
        FROM Test_Scores
    ...
    $stmt = $conn->prepare("
        SELECT MAX(marks_obtained) AS highest_marks, AVG(marks_obtained) AS average_marks, MAX(accuracy) AS highest_accuracy, AVG(accuracy) AS average_accuracy
        FROM Test_Scores
        WHERE testname = ?
    ");
    ...
    if ($marksRow = $marksResult->fetch_assoc()) {
        ...
        $highestAccuracy = $marksRow['highest_accuracy'];
        $averageAccuracy = $marksRow['average_accuracy'];
    }
    $stmt->close();
}
...
<body>
                    ...
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <p class="card-text">
                                ...
                                <strong>Award for Right:</strong> <?php echo htmlspecialchars($score['award_for_right']); ?><br>
                                <strong>Accuracy:</strong> <?php echo number_format($score['accuracy'], 2); ?>%<br>
                            </p>
                        </div>
                        <div class="card-footer">
                            ...
                            <button onclick="showAnalytics('<?php echo $highestMarks; ?>', '<?php echo $averageMarks; ?>', '<?php echo $score['marks_obtained']; ?>', '<?php echo $highestAccuracy; ?>', '<?php echo $averageAccuracy; ?>', '<?php echo $score['accuracy']; ?>')" class="btn btn-info">Analytics</button>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
    ...
    <script>
        ...
        var accuracyChart; // Global variable to hold the accuracy chart instance

        function showAnalytics(highestMarks, averageMarks, userMarks, highestAccuracy, averageAccuracy, userAccuracy) {
            ...
            if (accuracyChart) {
                accuracyChart.destroy(); // Destroy the existing chart instance if exists
            }
            accuracyChart = new Chart(ctxAccuracy, {
                type: 'bar',
                data: {
                    labels: ['Highest Accuracy', 'Average Accuracy', 'Your Accuracy'],
                    datasets: [{
                        label: 'Accuracy Performance',
                        data: [highestAccuracy, averageAccuracy, userAccuracy],
                        backgroundColor: ['rgba(54, 162, 235, 0.6)', 'rgba(255, 206, 86, 0.6)', 'rgba(255, 99, 132, 0.6)'],
                        borderColor: ['rgba(54, 162, 235, 1)', 'rgba(255, 206, 86, 1)', 'rgba(255, 99, 132, 1)'],
                        borderWidth: 1
                    }]
